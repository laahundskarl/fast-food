...
echo "[0/10] Configurando kubectl para o cluster EKS..."
...
echo "[5/11] Deploy da API..."
kubectl apply -f 05-api-deployment.yaml

echo "[6/11] Aguardando API ficar pronta..."
kubectl wait --for=condition=Ready pod -l app=fastfood-api --timeout=300s

echo "[7/11] Executando migrations do banco de dados..."
...
echo "[8/11] Instalando Metrics Server oficial..."
kubectl apply -f https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml

echo "[9/11] Aguardando Metrics Server ficar pronto..."
kubectl wait --for=condition=Ready pod -l k8s-app=metrics-server -n kube-system --timeout=300s

echo "[10/11] Deploy do serviço da API e LoadBalancer..."
kubectl apply -f 06-api-service.yaml
kubectl apply -f 07-loadbalancer.yaml

echo "[11/11] Deploy do HPA..."
